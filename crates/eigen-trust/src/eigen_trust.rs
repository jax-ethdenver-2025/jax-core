...
{
    pub fn new(current_node: F::NodeId, trust_fetcher: F) -> Self {
        let mut peers = HashSet::new();
        peers.insert(current_node.clone());
    ...
    pub fn remove_peer(&mut self, peer_id: F::NodeId) -> &mut Self {
        if peer_id != self.current_node {
            self.peers.remove(&peer_id);
            self.local_trust.remove(&peer_id);
            self.trust_cache.retain(|&(i, j), _| i != peer_id && j != peer_id);
    ...
    async fn get_trust(&mut self, i: &F::NodeId, j: &F::NodeId) -> Result<f64> {
        ...
        if i == &self.current_node {
            return Ok(*self.local_trust.get(j).unwrap_or(&0.0));
        }
    ...
    pub async fn compute_global_trust(&mut self) -> Result<HashMap<F::NodeId, f64>> {
        ...
        let peer_to_index: HashMap<F::NodeId, usize> = peers_vec.iter()
            .enumerate()
            .map(|(idx, peer_id)| (peer_id.clone(), idx))
            .collect();
        
        let index_to_peer: HashMap<usize, F::NodeId> = peers_vec.iter()
            .enumerate()
